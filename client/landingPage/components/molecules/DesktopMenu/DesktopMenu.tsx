...
export const DesktopMenu = ({ activeItem, setActiveItem, resetMenu, scrollToTop, isMobile, variant }: IMenuProps) => {
  const menuRef = useRef<HTMLUListElement | null>(null);
  const prevActiveItem = useRef<null>(null);
  const [hoverData, setHoverData] = useState(INIT_HOVER_DATA);
  ...
  useEffect(() => {
    const element = menuRef && menuRef.current;
    if (element && !isMobile) {
      const delayReset = debounce(() => {
        resetMenu();
        setHoverData(INIT_HOVER_DATA);
      }, 200);
      element.addEventListener('mouseleave', delayReset);
      return () => element.addEventListener('mouseleave', resetMenu);
    }
  }, []);
  ...
  const props = { activeItem, setPositionData, variant, resetMenu };
  ...
  return (
    <S.Menu ref={menuRef}>
      ...
      <S.Item onMouseOver={resetMenu}>
        <Link className="gtm_integrations" href={''}>
